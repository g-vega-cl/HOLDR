...
API.interceptors.request.use((req: any) => {
  if (localStorage.getItem("profile")) {
    req.headers.Authorization = `Bearer ${
      JSON.parse(localStorage.getItem("profile") || JSON.stringify(u)).token
    }`;
  }
...
export const updatePost = (id: any, updatedPost: any) =>
  API.patch(`/tutorial}/${id}`, updatedPost);
...
export const fetchUserTransactions = (email: any) =>
  API.get(`/payment/user/`, {
    params: {
      email: email,
    },
  });
export const createTransaction = (newTransaction: any) =>
  API.post("/payment", newTransaction);
